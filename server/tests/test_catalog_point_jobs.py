...
from server.app.domains.catalog_tree.teacher_search import teacher_search_document_sync_hash
...
def test_queue_teacher_search_sync_job_uses_independent_target_and_allows_directories() -> None:
    ...
    assert params["run_after_seconds"] == jobs.SOFT_SEARCH_SYNC_QUIET_SECONDS == 30
    assert params["max_coalesce_seconds"] == jobs.SOFT_SEARCH_SYNC_MAX_WAIT_SECONDS == 180


def test_catalog_jobs_only_allow_teacher_search_and_textbook_rag_work() -> None:
    assert jobs.JOB_TYPES == {
...
class _FakeDbSession:
    ...
    def execute(self, statement: Any, params: dict[str, Any] | None = None) -> _Result:
        if "SELECT document_hash FROM experiment_catalog_teacher_search_index_state" in str(statement):
...
def test_process_teacher_search_job_skips_upsert_when_document_hash_is_unchanged(monkeypatch) -> None:
    document = {"id": "cat-point-1", "title": "氯水 + KI", "updated_at": "2026-06-22T00:00:00"}
    sync_hash = teacher_search_document_sync_hash(document)
    client = _FakeIndexClient()
    marks: list[dict[str, Any]] = []

    monkeypatch.setattr(jobs, "db_session", lambda: _fake_db_session(sync_hash))
    monkeypatch.setattr("server.app.domains.catalog_tree.teacher_search.teacher_search_document_for_node", lambda *_args, **_kwargs: document)
    monkeypatch.setattr("server.app.domains.catalog_tree.teacher_search.configured_teacher_search_client", lambda: client)
    monkeypatch.setattr("server.app.domains.catalog_tree.teacher_search.mark_teacher_search_state_success", lambda **kwargs: marks.append(kwargs))

    result = jobs._process_teacher_search_job(jobs.CatalogPointJob(id="job-1", node_id="cat-point-1", job_type="teacher_search_upsert", attempts=1, payload={}))
...
def test_process_teacher_search_job_upserts_when_document_hash_changes(monkeypatch) -> None:
    document = {"id": "cat-point-1", "title": "氯水 + KI", "updated_at": "2026-06-22T00:00:00"}
    sync_hash = teacher_search_document_sync_hash(document)
    client = _FakeIndexClient()
    marks: list[dict[str, Any]] = []
    settings = type("Settings", (), {"teacher_catalog_search_analyzer": "ik_max_word"})()
...
def test_process_teacher_search_job_deletes_when_target_is_no_longer_searchable(monkeypatch) -> None:
    client = _FakeIndexClient()
    marks: list[dict[str, Any]] = []

    monkeypatch.setattr(jobs, "db_session", lambda: _fake_db_session("old-hash"))
    monkeypatch.setattr("server.app.domains.catalog_tree.teacher_search.teacher_search_document_for_node", lambda *_args, **_kwargs: None)
    monkeypatch.setattr("server.app.domains.catalog_tree.teacher_search.configured_teacher_search_client", lambda: client)
    monkeypatch.setattr("server.app.domains.catalog_tree.teacher_search.mark_teacher_search_state_success", lambda **kwargs: marks.append(kwargs))

    result = jobs._process_teacher_search_job(jobs.CatalogPointJob(id="job-1", node_id="cat-point-1", job_type="teacher_search_upsert", attempts=1, payload={}))

    assert result["reason"] == "node_not_searchable"
