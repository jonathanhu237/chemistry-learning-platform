...
def _point_content_source_refs(point_contexts: list[dict[str, Any]]) -> list[dict[str, Any]]:
    refs: list[dict[str, Any]] = []
    for index, context in enumerate(point_contexts, start=1):
        node_id = str(context.get("catalog_node_id") or context.get("placement_node_id") or "").strip()
        title = str(context.get("point_title") or "").strip() or node_id
        principle = str(context.get("principle") or "").strip()
        phenomenon = str(context.get("phenomenon_explanation") or "").strip()
        safety = str(context.get("safety_note") or "").strip()
        text_parts = [
            f"点位：{title}",
            f"目录路径：{' / '.join(str(item) for item in (context.get('catalog_path') or []) if str(item).strip())}",
            f"原理：{principle}",
            f"现象：{phenomenon}",
            f"安全：{safety}",
        ]
        refs.append(
            {
                "chunk_id": f"legacy-point-content:{node_id}:{index}",
                "source_boundary": "legacy_point_content",
                "source_file": "catalog_point_content",
                "section_title": title,
                "text": "\n".join(part for part in text_parts if part and not part.endswith("：")),
                "point_node_id": node_id,
                "point_node_ids": [node_id] if node_id else [],
                "canonical_point_id": context.get("canonical_point_id"),
...
def _local_generated_point_content_questions(
    *,
    experiment: dict[str, Any],
    request: GenerationRequest,
    source_refs: list[dict[str, Any]],
    ...
    valid_types = [item for item in request.question_types if item in OBJECTIVE_TYPES] or ["single_choice"]
    contexts = point_contexts or [{"point_title": experiment["title"], "principle": "", "phenomenon_explanation": "", "safety_note": ""}]
    questions: list[dict[str, Any]] = []
    for index in range(request.count):
        q_type = valid_types[index % len(valid_types)]
        context = contexts[index % len(contexts)]
        title = str(context.get("point_title") or experiment["title"]).strip()
        principle = str(context.get("principle") or "").strip()
        phenomenon = str(context.get("phenomenon_explanation") or "").strip()
        safety = str(context.get("safety_note") or "").strip()
        common = {
            ...
            questions.append(
                {
                    **common,
                    "question_type": "single_choice",
                    "stem": f"围绕“{title}”进行课堂测评时，下列哪项最适合作为判断依据？",
                    "options": [
                        {"label": "A", "text": phenomenon or principle or "结合点位原理、实验现象和安全提示综合判断。"},
                        {"label": "B", "text": "只依据试剂名称，不记录实验条件和现象。"},
                        {"label": "C", "text": "忽略安全提示，直接扩大实验操作规模。"},
                        {"label": "D", "text": "不核对点位资料，套用无关实验结论。"},
                    ],
                    "answer": {"value": "A"},
                    "explanation": f"该题基于点位资料生成，需把{title}的原理、现象与安全要求对应起来。",
                }
            )
        elif q_type == "true_false":
            questions.append(
                {
                    **common,
                    "question_type": "true_false",
                    "stem": f"判断：学习“{title}”时，只观察一个现象就可以得出完整结论。",
                    "options": [],
                    "answer": {"value": False},
                    "explanation": "实验结论需要同时结合原理、现象和条件控制，不能只依赖单一现象。",
                }
            )
        else:
            answer = safety or principle or phenomenon or title
            questions.append(
                {
                    **common,
                    "question_type": "fill_blank",
                    "stem": f"“{title}”的学习记录应至少包含实验原理、现象观察和________。",
                    "options": [],
                    "answer": {"accepted_answers": ["安全提示", "安全注意事项", answer], "match": "normalized_exact"},
                    "explanation": "点位资料由原理、现象和安全三类信息组成，题目用于检查学生是否完整记录。",
                }
            )
    return questions
...
def question_payload_has_catalog_evidence_lineage(
    ...
    *,
    ...
    if evidence_contract not in {"catalog_node_evidence", "legacy_point_content"}:
        return False
    if "legacy" in evidence_source and evidence_contract != "legacy_point_content":
        return False
    ...
    if not payload_node_ids and not payload_canonical_ids:
        return False
    if evidence_contract == "legacy_point_content":
        return bool(payload_node_ids and (source_audit.get("point_content_fields") or payload.get("source_refs") or lineage.get("source_refs")))
    return bool(source_audit.get("canonical_chunk_ids") or payload.get("source_refs") or lineage.get("source_refs"))


def attach_legacy_point_content_lineage(
    payload: dict[str, Any],
    *,
    source_refs: list[dict[str, Any]],
    target_points: list[dict[str, Any]] | None,
    ...
    source_audit = {
        "evidence_sufficient": bool(source_refs),
        "evidence_contract": "legacy_point_content",
        "evidence_source": "legacy_point_content",
        ...
        "canonical_chunk_ids": [item["chunk_id"] for item in source_refs if item.get("chunk_id")],
        "point_content_fields": ["principle", "phenomenon_explanation", "safety_note"],
        "reviewer_note": "Generated from catalog point content only; no RAG retrieval was used.",
    }
    ...
    metadata["catalog_point_contexts"] = point_contexts
    return {
        **payload,
...
def generate_legacy_point_content_question_drafts(
    *,
    payload: GenerationRequest,
    user: Any,
) -> dict[str, Any]:
    with db_session() as session:
        ...
        if not target_points:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail="Legacy question generation requires target catalog point node ids.",
            )
        target_point_node_ids = _target_point_node_ids(target_points)
        point_contexts = catalog_point_generation_contexts(session, target_points=target_points)
        source_refs = _point_content_source_refs(point_contexts)
        warning = "" if source_refs else "当前点位资料为空，已使用点位标题生成草稿，发布前必须人工核验。"
        ai_settings = effective_ai_settings(get_settings())
        generated = _try_openai_generation(
            experiment=experiment,
            request=payload,
        ...
        mode = "legacy_point_content_openai" if generated else "legacy_point_content_template"
        if not generated:
            generated = _local_generated_point_content_questions(
                experiment=experiment,
                request=payload,
                source_refs=source_refs,
                point_contexts=point_contexts,
        ...
        generation_id = str(
            session.execute(
                ...
                ),
                {
                    ...
                    "difficulty": payload.difficulty,
                    "requested_count": payload.count,
                    "provider": "openai" if mode == "legacy_point_content_openai" else "local",
                    "model": ai_settings.agent_llm_model or os.getenv("OPENAI_MODEL", ""),
                    "mode": mode,
                    "rag_sources": _json_array(source_refs),
                    "warning": warning,
        ...
        for row in generated[: payload.count]:
            ...
            row_payload = attach_legacy_point_content_lineage(
                row_payload,
                source_refs=source_refs,
                target_points=target_points,
                point_contexts=point_contexts,
    ...
    return {
        ...
        "source_refs": source_refs,
        "evidence_package": {
            "mode": "legacy_point_content",
            "evidence_contract": "legacy_point_content",
            "source_mode": "catalog_point_content",
            "target_point_node_ids": target_point_node_ids,
