...
from server.app.domains.feedback.teacher_feedback import (
    feedback_summary,
...
router = APIRouter(prefix="/api/teacher", tags=["teacher-feedback"])


@router.get("/feedback/summary", response_model=FeedbackSummaryResponse)
async def teacher_feedback_summary(
...
async def teacher_get_feedback(
...
async def teacher_get_feedback_attachment(
    feedback_id: str = Path(min_length=1),
...
async def teacher_update_feedback(
