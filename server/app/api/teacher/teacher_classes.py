...
router = APIRouter(prefix="/api/teacher", tags=["teacher-classes"])


@router.get("/classes", response_model=list[ClassResponse])
async def teacher_list_classes(user: AuthUser = Depends(require_teacher_user)) -> list[ClassResponse]:
    return list_classes(user)


@router.get("/registration-settings", response_model=RegistrationSettingsResponse)
async def teacher_get_registration_settings(
...
async def teacher_update_registration_settings(
...
async def teacher_get_class_registration_settings(
...
async def teacher_update_class_registration_settings(
    payload: RegistrationSettingsUpdateRequest,
    class_id: str = Path(min_length=1),
    user: AuthUser = Depends(require_teacher_user),
) -> RegistrationSettingsResponse:
...
async def teacher_get_class_smart_assessment_strategy(
    class_id: str = Path(min_length=1),
    user: AuthUser = Depends(require_teacher_user),
) -> SmartAssessmentStrategyResponse:
...
async def teacher_update_class_smart_assessment_strategy(
...
async def teacher_clear_class_smart_assessment_strategy(
    class_id: str = Path(min_length=1),
    user: AuthUser = Depends(require_teacher_user),
) -> SmartAssessmentStrategyResponse:
...
async def teacher_get_class_smart_assessment_preview(
    class_id: str = Path(min_length=1),
    user: AuthUser = Depends(require_teacher_user),
) -> SmartAssessmentClassPreviewResponse:
...
async def teacher_get_class_custom_assessment_settings(
    class_id: str = Path(min_length=1),
    user: AuthUser = Depends(require_teacher_user),
) -> CustomAssessmentSettingsResponse:
...
async def teacher_update_class_custom_assessment_settings(
...
async def teacher_clear_class_custom_assessment_settings(
    class_id: str = Path(min_length=1),
    user: AuthUser = Depends(require_teacher_user),
) -> CustomAssessmentSettingsResponse:
...
async def teacher_get_class_assessment_report_prompts(
    class_id: str = Path(min_length=1),
    user: AuthUser = Depends(require_teacher_user),
) -> AssessmentReportPromptSettingsResponse:
...
async def teacher_update_class_assessment_report_prompts(
...
async def teacher_clear_class_assessment_report_prompts(
    class_id: str = Path(min_length=1),
    user: AuthUser = Depends(require_teacher_user),
) -> AssessmentReportPromptSettingsResponse:
...
async def teacher_list_student_assessment_reports(
    class_id: str = Path(min_length=1),
...
async def teacher_get_student_assessment_report(
    class_id: str = Path(min_length=1),
    student_id: str = Path(min_length=1),
...
async def teacher_create_class(
...
async def teacher_update_class(
...
async def teacher_assign_teacher_to_class(
...
async def teacher_preview_roster_import(
    class_id: str = Path(min_length=1),
...
async def teacher_import_roster(
    class_id: str = Path(min_length=1),
    file: UploadFile = File(...),
...
async def teacher_list_roster_students(
    class_id: str = Path(min_length=1),
    user: AuthUser = Depends(require_teacher_user),
...
async def teacher_create_roster_student(
...
async def teacher_update_roster_student(
...
async def teacher_disable_roster_student(
    class_id: str = Path(min_length=1),
    student_id: str = Path(min_length=1),
    user: AuthUser = Depends(require_teacher_user),
) -> RosterStudentResponse:
