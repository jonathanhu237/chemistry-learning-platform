...
from server.app.experiment_teacher_schemas import WorkbenchMessageRequest, WorkbenchSessionRequest
...
async def teacher_create_question_workbench_session(
...
async def teacher_get_question_workbench_session(
...
async def teacher_clear_question_workbench_evidence_cache(
    session_id: str = Path(min_length=1),
    user: AuthUser = Depends(require_teacher_user),
) -> dict[str, Any]:
...
async def teacher_stream_question_workbench_message(
...
async def teacher_send_question_workbench_message(
    payload: WorkbenchMessageRequest,
    session_id: str = Path(min_length=1),
    user: AuthUser = Depends(require_teacher_user),
) -> dict[str, Any]:
...
async def teacher_reject_question_workbench_candidate(
...
async def teacher_publish_question_workbench_candidate(
    candidate_id: str = Path(min_length=1),
    user: AuthUser = Depends(require_teacher_user),
) -> dict[str, Any]:
