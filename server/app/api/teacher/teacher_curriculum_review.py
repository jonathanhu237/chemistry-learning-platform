...
router = APIRouter(prefix="/api/teacher", tags=["teacher-curriculum-review"])
...
async def teacher_list_curriculum_versions(
...
async def teacher_create_curriculum_version(
...
async def teacher_get_curriculum_version(
...
async def teacher_publish_curriculum_version(
    version_id: str = Path(min_length=1),
    user: AuthUser = Depends(require_teacher_user),
) -> dict[str, Any]:
    try:
...
async def teacher_archive_curriculum_version(
    version_id: str = Path(min_length=1),
    user: AuthUser = Depends(require_teacher_user),
) -> dict[str, Any]:
    try:
...
async def teacher_list_review_items(
...
async def teacher_get_review_item(
...
async def teacher_apply_review_action(
