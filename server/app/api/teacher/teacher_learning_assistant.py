...
from server.app.domains.assistant.adapters.teacher_debug import run_teacher_debug_agent, stream_teacher_debug_agent
...
router = APIRouter(prefix="/api/teacher", tags=["teacher-learning-assistant"])
...
async def teacher_get_learning_assistant_runtime(
    user: AuthUser = Depends(require_teacher_user),
) -> dict[str, Any]:
    ai_config = get_ai_configuration_response(can_edit=is_teacher_role(user.role), auto_check=False)
...
async def teacher_test_learning_assistant(
    ...
    return await run_teacher_debug_agent(
        payload,
        user,
...
async def teacher_stream_learning_assistant(
    payload: LearningAssistantAskRequest,
    ...
    async def event_stream():
        async for item in stream_teacher_debug_agent(
            payload,
            user,
            settings=effective_ai_settings(get_settings()),
            rag_access_enabled=bool(ai_config.enabled_features.rag_access_enabled),
