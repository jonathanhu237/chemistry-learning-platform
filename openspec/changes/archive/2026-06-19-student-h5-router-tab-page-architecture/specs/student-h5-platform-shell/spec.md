...
- **WHEN** a browser or WebView requests a student H5 detail route such as `/chapter/{profileId}`, `/point/{experimentId}`, `/ai/chat`, `/assessment/session/{sessionId}`, `/assessment/report/{sessionId}`, or `/feedback/new`
- **THEN** the FastAPI service MUST serve the student SPA entrypoint when the student frontend build exists
