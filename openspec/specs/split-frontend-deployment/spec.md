...
The system SHALL deploy the student H5 frontend and teacher/admin frontend as independent frontend services rather than serving their SPA bundles from the backend service.

#### Scenario: Compose starts the full application
- **WHEN** the default production-like Compose application is started
- **THEN** `student-web` MUST be a running service with its own published port
- **AND** `admin-web` MUST be a running service with its own published port
...
- **WHEN** a browser requests a deep student or admin SPA route from the corresponding frontend service
...
- **THEN** it MUST NOT copy `apps/admin-web/dist`
- **AND** it MUST NOT copy `apps/student-web/dist`.
...
#### Scenario: Admin frontend calls API
- **WHEN** the admin frontend issues a request to `/api/*`
- **THEN** the admin frontend runtime MUST forward the request to the backend service
- **AND** the browser-facing API contract MUST remain `/api/*`.

### Requirement: Compose validation includes frontend services
The Compose smoke validation SHALL treat the student and admin frontend services as required application services.

#### Scenario: Compose smoke runs
- **WHEN** Compose smoke validation runs for the full application
- **THEN** it MUST verify `backend`, `student-web`, `admin-web`, `postgres`, `elasticsearch`, `tusd`, and `video-worker` are running
