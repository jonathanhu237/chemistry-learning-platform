# admin-web-engineering-structure Specification

## Purpose
TBD - created by archiving change standardize-application-engineering-structure. Update Purpose after archive.
## Requirements
### Requirement: Teacher/admin shell is separated from feature pages
The teacher/admin frontend SHALL separate app providers, auth guard, route registry, navigation model, shell layout, and feature pages.
...
#### Scenario: A new admin page is added
- **WHEN** a new teacher/admin page is introduced
...
### Requirement: Teacher/admin features are business capability modules
Teacher/admin frontend features SHALL be grouped by business capability and decomposed into page orchestration, hooks, panels, forms, tables, and adapters when they grow.
...
### Requirement: Teacher/admin API clients are domain-owned
The teacher/admin frontend SHALL split HTTP primitives from domain-specific clients and schemas.
...
### Requirement: Teacher/admin structure changes keep e2e smoke
Teacher/admin shell and top-level page structure changes SHALL keep admin e2e smoke as a release gate.
